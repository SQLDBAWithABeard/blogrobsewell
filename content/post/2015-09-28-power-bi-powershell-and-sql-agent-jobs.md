...
GO
```  
The Detail table holds the results of every Agent Job and the Server table holds a roll up for each server. The script to gather this information is based on the script I used to put the information into an Excel Sheet as described in my post [How I Check Hundreds of Agent Jobs inundefinedSeconds with PowerShell](https://blog.robsewell.com/blog/how-i-check-hundreds-of-sql-agent-jobs-in-60-seconds-with-powershell/) which I also altered to send an HTML email to the DBA team each morning. This however is a much better solution and allows for better monitoring and trending.
...
```  
My friend Terry McCann [b](http://hyperbi.co.uk) | [t](https://twitter.com/@sqlshark) helped me create a column that returns true if the last run time is withinundefinedhours of the current time to help identify the recent jobs that have failed NOTE - On a Monday morning you will need to change this if you do not check your jobs on the weekend.
  ```
...
, 0)
```  
Next we have to create some measures for the sum of failed jobs and the averages This is the code for```
Select IL.InstanceID,
IL.ServerName,
IL.InstanceName,
IL.Environment,
...
```  
I did the same forundefineddays. I used the [TechNet reference for DAX expressions](http://social.technet.microsoft.com/wiki/contents/articles/680.powerpivot-dax-filter-functions.aspx?WT.mc_id=DP-MVP-5002693) and got ideas from [Chris Webbs blog](http://blog.crossjoin.co.uk/category/dax/)

[![ag6](https://sqldbawithabeard.com/wp-content/uploads/2015/09/ag6.jpg?w=83)](https://sqldbawithabeard.com/wp-content/uploads/2015/09/ag6.jpg) First I created theundefinedday historical trend chart using a Line and Clustered column chart using the last run date as the axis and the succeed measure as the column and the Failed, Failed```
Select IL.InstanceID,
IL.ServerName,
IL.InstanceName,
IL.Environment,
...
ON IL.InstanceID = AJD.InstanceID
WHERE LastRunTime > DATEADD(Day,-31,GETDATE())
```Day Average and failedundefinedday average as the lines
...
The rest of the report is measures for```
Select IL.InstanceID,
IL.ServerName,
IL.InstanceName,
IL.Environment,
...
ON IL.InstanceID = AJD.InstanceID
WHERE LastRunTime > DATEADD(Day,-31,GETDATE())
```day average andundefinedday average, a slicer for environment  and two tables, one to show the historical job counts and one to show the jobs that have failed in the lastundefinedhours using the Last Run Relative Hour measure from above
...
[**Using Power Bi with my DBA Database**](/blog/using-power-bi-with-my-dba-database/ )
