...
<P>There are onlyundefinedparameters Sqlserver, Credential and detailed</P>
...
<P>This time we get the server name, server collation, database name , database collation and the IsEqual property. This is a collection of objects so we are not bound be just seeing them on the screen we can use them as <A href="/blog/taking-dbatools-test-dbalastbackup-a-little-further/">I blogged about here</A></P>
<P>For example</P>
<DIV>
...
<DIV></DIV>
<P>Excellent, that works a treat. How about Linux? Does this work if SQL is running on Linux? We will have to use the credential parameter as we need SQL Authentication. this time I have used the <A href="https://msdn.microsoft.com/en-us/powershell/reference/5.1/microsoft.powershell.utility/format-table?WT.mc_id=DP-MVP-5002693" target=_blank>Format-Table </A>command to format the output.</P>
<DIV>
<DIV>
...
<DIV></DIV>
<P>Lets add some <A href="https://github.com/pester?WT.mc_id=DP-MVP-5002693" target=_blank>Pester</A> tests. If we want to test a list of servers and see if any of their databases have an incorrect collation we can simply test if the IsEquals flag contains a false.</P>
<DIV></DIV>
