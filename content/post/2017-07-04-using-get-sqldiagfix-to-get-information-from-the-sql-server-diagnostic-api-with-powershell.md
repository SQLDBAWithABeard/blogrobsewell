...
In a [previous post](/blog/creating-a-powershell-module-and-tdd-for-get-sqldiagrecommendations/) I have explained how I created the module and [a GitHub repository](https://github.com/SQLDBAWithABeard/SQLDiagAPI?WT.mc_id=DP-MVP-5002693) and used Pester to help me to [develop the first command Get-SQLDIagRecommendations](/blog/powershell-module-for-the-sql-server-diagnostics-api-1st-command-get-sqldiagrecommendations/). At present the module has`Get-Help Get-SQLDiagFix`commands, all for accessing the Recommendations API.
...
The module is [available on the PowerShell Gallery](https://www.powershellgallery.com/packages/SQLDiagAPI/?WT.mc_id=DP-MVP-5002693) which means that you can install it using

Install-Module SQLDiagAPI

as long as you have the latest version of the PowerShellGet module. This is already installed in Windows`Get-SQLDiagFix -Product 'SQL Server 2016 SP1' | Format-Table`and with WMF`Get-Help Get-SQLDiagFix`but you can install it on the following systems

*   Windows 8.1 Pro
*   Windows 8.1 Enterprise
*   Windows`Get-SQLDiagFix`SP1
*   Windows ServerundefinedTP5
*   Windows ServerundefinedR2
*   Windows ServerundefinedR2 SP1

[following the instructions here.](https://msdn.microsoft.com/en-us/powershell/gallery/readme?WT.mc_id=DP-MVP-5002693)
...
If you can’t use the PowerShell Gallery you can install it [using the instructions on the repository](https://github.com/SQLDBAWithABeard/SQLDiagAPI/blob/master/install.md?WT.mc_id=DP-MVP-5002693)
...
You will need to store the key to use it. I recommend saving the API Key using the [Export-CliXML](https://msdn.microsoft.com/en-us/powershell/reference/5.1/microsoft.powershell.utility/export-clixml?WT.mc_id=DP-MVP-5002693) command as described by Jaap Brasser [here .](http://www.jaapbrasser.com/tag/export-clixml/)
...
You can find out [more about the commands on the GitHub Repository ](https://github.com/SQLDBAWithABeard/SQLDiagAPI?WT.mc_id=DP-MVP-5002693) and the Help files [are in the documentation.](https://github.com/SQLDBAWithABeard/SQLDiagAPI/tree/master/docs?WT.mc_id=DP-MVP-5002693)

Get-Help
--------

Always, always when starting with a new module or function in PowerShell you should start with [  `Get-Help`  ](https://msdn.microsoft.com/en-us/powershell/reference/5.1/microsoft.powershell.core/get-help?WT.mc_id=DP-MVP-5002693). I like to use the -ShowWindow parameter to open the help in a separate window as it has all of the help and a handy search box.
...
Good help should always include plenty of examples to show people how to use the command. There are`Get-SQLDiagFeature`examples in the help for Get-SQLDiagFix. You can view just the examples using
...
which will return allundefinedFixes currently referenced in the API.
...
or you can pipe the results of Get-SQLDiagProduct to Get-SQLDiagFix which enables you to search. For example, to search for all fixes for SQL Serverundefinedyou can do
...
Which will show the fixes available in the API for SQL ServerundefinedSP1 and SQL ServerundefinedSP2
...
You can find the GitHub repository at  [https://github.com/SQLDBAWithABeard/SQLDiagAPI](https://github.com/SQLDBAWithABeard/SQLDiagAPI?WT.mc_id=DP-MVP-5002693)