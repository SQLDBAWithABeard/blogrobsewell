...
---
<P>I have written a module <A href="https://github.com/SQLDBAWithABeard/SQLDiagAPI?WT.mc_id=DP-MVP-5002693" rel=noopener target=_blank>SQLDiagAPI</A> for consuming the <A href="https://ecsapi.portal.azure-api.net/" rel=noopener target=_blank>SQL Server Diagnostics API</A> with PowerShell. I blogged about how I used Pester to <A href="/blog/creating-a-powershell-module-and-tdd-for-get-sqldiagrecommendations/" rel=noopener target=_blank>develop one of the functions .</A> Whilst writing <A href="/blog/using-get-sqldiagfix-to-get-information-from-the-sql-server-diagnostic-api-with-powershell/" rel=noopener target=_blank>Get-SQLDiagFix</A> I wrote some Pester Tests to make sure that the output from the code was as expected.</P>
<H2>Pester</H2>
...
<P>I can now test that the code I have written for Get-SQLDiagFix returns the correct data without any parameters using this test with <A href="https://msdn.microsoft.com/en-us/powershell/reference/5.1/microsoft.powershell.utility/compare-object?WT.mc_id=DP-MVP-5002693" rel=noopener target=_blank>Compare-Object</A>.</P><PRE class="lang:ps decode:true">It "returns all of the fixes with no parameter" {
...
<UL>
<LI>SQL ServerundefinedSP3 
<LI>SQL ServerundefinedSP1 
<LI>SQL ServerundefinedRTM 
<LI>SQL ServerundefinedSP1 
<LI>SQL ServerundefinedSP2 </LI></UL>
<DIV></DIV>
...
<DIV></DIV>
<DIV><PRE class="lang:ps decode:true">$TestCases = @{ ProductName = 'SQL ServerundefinedSP3'},
@{ ProductName = 'SQL ServerundefinedSP1'},
@{ ProductName = 'SQL ServerundefinedRTM'},
@{ ProductName ='SQL ServerundefinedSP1'},
@{ ProductName = 'SQL ServerundefinedSP2'}
...
<P>I also want to test that Get-SQLDiagFix will work for multiple Products. I need to create TestCases for those too. I do that in exactly the same way</P><PRE class="lang:ps decode:true">$TestCases = @{ ProductName = 'SQL ServerundefinedSP3', 'SQL ServerundefinedSP1'},
@{ ProductName = 'SQL ServerundefinedSP3', 'SQL ServerundefinedSP1', 'SQL ServerundefinedRTM'},
@{ ProductName = 'SQL ServerundefinedSP3', 'SQL ServerundefinedSP1', 'SQL ServerundefinedRTM', 'SQL ServerundefinedSP1'},
@{ ProductName = 'SQL ServerundefinedSP3', 'SQL ServerundefinedSP1', 'SQL ServerundefinedRTM', 'SQL ServerundefinedSP1', 'SQL ServerundefinedSP2'}
...
<P>I createdundefinedtest cases. Each one has a random number of features betweenundefinedand the number of features.&nbsp; I can then write one test to make use of those test cases. This is how I do that</P><PRE class="lang:ps decode:true">## GenerateundefinedTestCases of a random number of Features
$TestCases = @()
$x = 10
While ($x -gt 0) {
...
<P>Now there areundefinedtests each with a random number of features and the results look like this. Each time the test is run it will use a different set of features for each of theundefinedtests but I will know that I am testing that the code will return the correct results for multiple features</P>
...
<P>To test for a single product and multiple features I use this code</P><PRE class="lang:ps decode:true"> foreach ($Product in $Products) {
    ## GenerateundefinedTestCases of a random number of Features
    $TestCases = @()
    $x = 10
    While ($x -gt 0) {
        ## We are testing multiples so we need at least 2
...
<P>I test for multiple products with a single feature and multiple products with multiple features like this</P><PRE class="lang:ps decode:true"> $Products = @('SQL ServerundefinedSP3', 'SQL ServerundefinedSP1'),
 @('SQL ServerundefinedSP3', 'SQL ServerundefinedSP1', 'SQL ServerundefinedRTM'),
 @('SQL ServerundefinedSP3', 'SQL ServerundefinedSP1', 'SQL ServerundefinedRTM', 'SQL ServerundefinedSP1'),
 @('SQL ServerundefinedSP3', 'SQL ServerundefinedSP1', 'SQL ServerundefinedRTM', 'SQL ServerundefinedSP1', 'SQL ServerundefinedSP2')
 ...
 $Products = @('SQL ServerundefinedSP3', 'SQL ServerundefinedSP1'),
 @('SQL ServerundefinedSP3', 'SQL ServerundefinedSP1', 'SQL ServerundefinedRTM'),
 @('SQL ServerundefinedSP3', 'SQL ServerundefinedSP1', 'SQL ServerundefinedRTM', 'SQL ServerundefinedSP1'),
 @('SQL ServerundefinedSP3', 'SQL ServerundefinedSP1', 'SQL ServerundefinedRTM', 'SQL ServerundefinedSP1', 'SQL ServerundefinedSP2')
 foreach ($Product in $Products) {
...
<P>You can see all of the unit tests for the SQLDiagAPI module <A href="https://github.com/SQLDBAWithABeard/SQLDiagAPI/blob/master/tests/Unit.Tests.ps1?WT.mc_id=DP-MVP-5002693" rel=noopener target=_blank>in my GitHub repository</A></P>
<P>The module is <A href="https://www.powershellgallery.com/packages/SQLDiagAPI/?WT.mc_id=DP-MVP-5002693" rel=noopener target=_blank>available on the PowerShell Gallery</A> which means that you can install it using</P><PRE class="lang:ps decode:true">Install-Module SQLDiagAPI</PRE>
