...
---
In my previous posts about [writing your first Pester Test](/blog/write-your-first-pester-test-today/) and [looping through instances](/blog/2-ways-to-loop-through-collections-in-pester/) I described how you can start to validate that your SQL Server is how YOU want it to be.
...
In this (made up) example we loop through```
...
```instances and try to check the DNS Server entry is correct but for one of them we get a massive error and if we had created a large number of tests for each machine we would have a large number of massive errors.
...
One way of dealing with empty collections is to test that they have more than```
if ($instances.count -gt 0) {
    $instances.ForEach{
        ## Tests in here
    }
...
We can use [ `Test-DbaConnection` ](https://dbatools.io/functions/test-dbaconnection/)to perform a check at the beginning of the loop as we discussed in the [previous post](/blog/2-ways-to-loop-through-collections-in-pester/)
 ```
$instances.ForEach{
