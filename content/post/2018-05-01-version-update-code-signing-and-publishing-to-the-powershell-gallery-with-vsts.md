...
First you need to create a VSTS project by going to [https://www.visualstudio.com/](https://www.visualstudio.com/?WT.mc_id=DP-MVP-5002693) This is free for up toundefinedusers withundefinedconcurrent CI/CD queue limited to a maximum ofundefinedminutes run time which should be more than enough for your PowerShell module.
...
You can use the [Pester Test Runner Build Task](https://marketplace.visualstudio.com/items?itemName=richardfennellBM.BM-VSTS-PesterRunner-Task&WT.mc_id=DP-MVP-5002693) from the folk at [Black Marble](http://blackmarble.com/) by clicking on the + sign next to Phaseundefinedand searching for Pester
...
Click on the + sign next to Phaseundefinedand search for Publish
...
Again choose an empty process and name it sensibly, click the + sign next to Phaseundefinedand search for PowerShell

[![19 - PowerShell task.png](/assets/uploads/2018/05/19-PowerShell-task.png)](/assets/uploads/2018/05/19-PowerShell-task.png)

I change the version toundefinedand use this code. Note that the commit message has \*\*\*NO_CI\*\*\* in it. Putting this in a commit message tells VSTS not to trigger a build for this commit.
...
Then you need to add a task to download the secure file. Click on the + sign next to Phaseundefinedand search for secure
