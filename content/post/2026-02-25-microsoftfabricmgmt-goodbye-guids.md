...
- **First lookup for a single workspace**: ~5s (makes```powershell
Get-PSFConfig -Module MicrosoftFabricMgmt -Name Cache.*
```or two additional API calls to resolve the GUIDs for friendly output)
...
This time it takes`ServerName`seconds to get the workspace, resolve the capacity name, get the lakehouses in the workspace, resolve the workspace names and then get all of the SQL Endpoints. The second time it only takes```
Successfully cleared 42 cached name resolution(s)
```seconds because all of the names are cached.
